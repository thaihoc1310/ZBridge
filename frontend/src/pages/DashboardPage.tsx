...
export function DashboardPage() {
  ...
  const mentionSettingsLink = can(PERMISSIONS.mentionPolicyManage) || can(PERMISSIONS.staffManage) || can(PERMISSIONS.mentionBulkApply);
  const cards = [
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <motion.div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: .08 } } }}>
        {cards.map(({ label, value, sub, icon: Icon, status, to }) => (
          <motion.article key={label} variants={{ hidden: { opacity: 0, y: 18 }, visible: { opacity: 1, y: 0 } }} className={`card group relative overflow-hidden p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover ${to ? "cursor-pointer" : ""}`}>
            ...
            <div className="relative flex items-start justify-between"><p className="text-sm font-medium text-muted-foreground">{label}</p><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent to-accent-secondary text-white shadow-sm"><Icon className="h-4 w-4" /></span></div>
            <div className="relative mt-4">{status && data ? <StatusBadge status={String(value)} /> : <p className="font-display text-3xl">{isLoading ? "···" : value}</p>}{sub && <p className="mt-1.5 text-xs text-muted-foreground">{sub}</p>}</div>
          </motion.article>
        ))}
      </motion.div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        {mentionSettingsLink && <Link to="/mention-settings" className="card group flex items-center justify-between p-5 transition hover:border-accent/30 hover:shadow-card-hover"><span><span className="text-sm font-semibold">Tag tên tự động</span><span className="mt-1 block text-xs text-muted-foreground">Nhân sự, chính sách AI và áp dụng hàng loạt</span></span><ArrowUpRight className="text-muted-foreground transition group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-accent" /></Link>}
      </div>
    </div>
  );
}
