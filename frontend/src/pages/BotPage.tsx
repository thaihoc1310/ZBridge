...
import { Bot, Link2, LogOut, QrCode, RefreshCw, ShieldCheck, UserRound, UsersRound } from "lucide-react";
...
export function BotPage() {
  ...
  const qr = useQuery({ queryKey: ["bot-qr"], queryFn: () => api<QRState>("/bot/qr"), enabled: qrOpen, refetchInterval: qrOpen ? 2_000 : false });
  ...
  return <div className="mx-auto max-w-6xl">
    <PageHeader eyebrow="Connection center" title="Zalo" highlight="Bot" description="Quản lý phiên đăng nhập riêng tư, kết nối và danh tính tài khoản Zalo." action={bot?.status === "CONNECTED" ? <Button variant="secondary" onClick={() => action.mutate("reconnect")} loading={action.isPending}><RefreshCw className="h-4 w-4" />Kết nối lại</Button> : <Button onClick={() => action.mutate("connect")} loading={action.isPending}><QrCode className="h-4 w-4" />Kết nối Zalo</Button>} />
    ...
    <div className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
      ...
      <aside className="dot-grid relative overflow-hidden rounded-3xl bg-foreground p-7 text-white shadow-xl sm:p-9">
        ...
        <Button variant="ghost" className="relative mt-10 w-full border border-white/10 text-slate-300 hover:bg-white/10 hover:text-white" disabled={!bot?.session_active} onClick={() => { if (confirm("Đăng xuất bot sẽ xóa session đã lưu. Tiếp tục?")) action.mutate("disconnect"); }}><LogOut className="h-4 w-4" />Đăng xuất bot</Button>
      </aside>
    </div>
