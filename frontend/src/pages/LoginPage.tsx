...
export function LoginPage() {
  ...
  return (
    <main className="grid min-h-screen bg-white lg:grid-cols-[1.05fr_.95fr]">
      <section className="relative hidden overflow-hidden bg-foreground p-12 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div><div className="font-display text-2xl">ZBridge</div><div className="font-mono text-[9px] uppercase tracking-[.25em] text-slate-400">Zalo operations</div></div>
        </div>
        <div className="relative z-10 max-w-xl">
          ...
          <p className="mt-6 max-w-lg text-lg leading-relaxed text-slate-300">ZBridge giúp doanh nghiệp nhỏ quản lý khách hàng, công nợ và các luồng nhắc việc tự động trong một nơi rõ ràng, dễ vận hành.</p>
          <motion.div className="relative mt-12 h-52 max-w-lg" initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .25 }}>
            ...
            <div className="absolute bottom-0 right-0 w-64 animate-float rounded-2xl border border-white/10 bg-white/10 p-5 shadow-2xl backdrop-blur-xl [animation-delay:1s]">
              <div className="flex items-center justify-between"><span className="text-sm text-slate-300">Khách hàng</span><span className="font-display text-3xl">103</span></div>
        ...
        <p className="relative z-10 text-xs text-slate-500">Secure internal gateway · Credentials never reach the browser</p>
      </section>
      ...
      <section className="flex items-center justify-center p-6 sm:p-12">
        <motion.div className="w-full max-w-md" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6 }}>
          ...
          <form onSubmit={submit} className="mt-9 space-y-5">
            ...
            {login.isError && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">{login.error instanceof ApiError ? login.error.message : "Không thể đăng nhập."}</div>}
          ...
          <div className="mt-8 flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 text-emerald-600" />Phiên đăng nhập được bảo vệ bằng HTTP-only cookie</div>
        </motion.div>
      </section>
    </main>
  );
