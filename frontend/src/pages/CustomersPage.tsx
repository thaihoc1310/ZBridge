...
export function CustomersPage() {
  ...
  useEffect(() => {
    if (!didAutoSync.current) { didAutoSync.current = true; sync.mutate(); }
        ...
        <table className="w-full min-w-[1120px] table-fixed border-collapse text-left">
          ...
          <tbody>
            ...
            {customers.data?.items.map((customer) => <tr key={customer.id} className="group border-b border-border transition-colors last:border-0 hover:bg-blue-50/40">
              ...
              <td className="px-5 py-4">{customer.folder_url ? <a href={customer.folder_url} target="_blank" rel="noreferrer" className="inline-flex max-w-full items-center gap-1.5 text-sm font-medium text-accent underline decoration-blue-200 underline-offset-4 transition hover:decoration-accent"><span className="truncate">Mở thư mục</span><ExternalLink className="h-3.5 w-3.5 shrink-0" /></a> : <button className="text-sm italic text-muted-foreground/70 transition hover:text-accent" onClick={() => setFolderCustomer(customer)}>+ Thêm thư mục</button>}</td>
              <td className="px-5 py-4"><DebtStatusOptions value={customer.has_debt} onChange={(nextValue) => setDebtConfirmation({ customer, nextValue })} /></td>
