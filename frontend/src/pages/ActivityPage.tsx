  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <section className="card overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        {view === "delivery" ? <div className="flex flex-wrap gap-2"><select className="field min-h-11 w-auto" value={deliveryStatus} onChange={(event) => setDeliveryFilter(event.target.value, today)}><option value="">Mọi trạng thái</option><option value="SENT">Thành công</option><option value="FAILED">Thất bại</option></select><label className="flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-border bg-white px-4 text-sm"><input type="checkbox" checked={today} onChange={(event) => setDeliveryFilter(deliveryStatus, event.target.checked)} className="h-4 w-4 accent-blue-600" />Chỉ hôm nay</label></div> : <select className="field min-h-11 w-auto" value={modelStatus} onChange={(event) => setModelFilter(event.target.value)}><option value="">Mọi trạng thái</option><option value="SUCCEEDED">Thành công</option><option value="FAILED">Lỗi model</option><option value="PROCESSING">Đang gọi</option></select>}
      </div>
