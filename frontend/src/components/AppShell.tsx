  ...
  return <div className="min-h-screen bg-background">
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-[1680px] items-center gap-6 px-4 sm:px-6 xl:px-8">
        <Link to="/" aria-label="Về trang chủ" className="group flex shrink-0 items-center gap-2.5">
        ...
        <div ref={accountRef} className="relative ml-auto hidden md:block">
          ...
          >
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-100 to-indigo-100 text-[10px] font-bold text-accent">{initials(displayName)}</span>
            <span className="min-w-0">
              <span className="block max-w-36 truncate text-xs font-semibold leading-tight">{displayName}</span>
              <span className="block max-w-36 truncate text-[10px] leading-tight text-muted-foreground">{user.role.name}</span>
            </span>
          </button>
          {accountOpen && <div role="menu" className="absolute right-0 top-14 z-30 w-72 rounded-2xl border border-border bg-white p-2 shadow-2xl">
            <div className="border-b border-border px-3 pb-3 pt-2">
              <p className="truncate text-sm font-semibold">{displayName}</p>
              <p className="mt-0.5 truncate text-xs text-muted-foreground">{user.email}</p>
              <span className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-semibold text-accent"><ShieldCheck className="h-3 w-3" />{user.role.name}</span>
            </div>
            <button type="button" role="menuitem" className="mt-1 flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm font-medium text-foreground transition hover:bg-muted" onClick={() => { setAccountOpen(false); setPasswordOpen(true); }}>
              <KeyRound className="h-4 w-4 text-accent" />Đổi mật khẩu
            </button>
            <button type="button" role="menuitem" className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm font-medium text-muted-foreground transition hover:bg-muted hover:text-foreground disabled:opacity-50" disabled={logout.isPending} onClick={() => logout.mutate()}>
      ...
      {open && <div className="border-t border-border bg-white p-3 shadow-xl md:hidden">
        ...
        <Button variant="ghost" className="mt-2 w-full justify-start text-muted-foreground" onClick={() => { setOpen(false); setPasswordOpen(true); }}><KeyRound className="h-4 w-4" />Đổi mật khẩu</Button>
        <Button variant="ghost" className="w-full justify-start text-muted-foreground" loading={logout.isPending} onClick={() => logout.mutate()}><LogOut className="h-4 w-4" />Đăng xuất</Button>
      </div>}
    </header>
