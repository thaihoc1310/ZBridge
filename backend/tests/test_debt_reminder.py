...
from app.services import debt_reminder_scheduler
...
def test_next_debt_reminder_repeats_and_preserves_monthly_anchor() -> None:
    ...
    assert next_debt_reminder_run(
        25, send_time, 3, august_25, has_debt=True
    ) == datetime(2026, 8, 28, 2, tzinfo=UTC)
    assert next_debt_reminder_run(
        25, send_time, 3, september_24, has_debt=True
    ) == datetime(2026, 9, 25, 2, tzinfo=UTC)
    assert next_debt_reminder_run(
        25, send_time, 3, august_25, has_debt=False
    ) == datetime(2026, 9, 25, 2, tzinfo=UTC)
